...
\problem*{6}
\question{Escreva um descrição informal e formal de uma máquina de Turing que decida as seguintes linguagens:}
	\begin{itemize}
		\item $L = \{a^ib^jc^k: i \times j = k,\ i,j,k \geq 1\}$
		
		\item $L = \{w zin \{0,1\}^*: w = w^R\}$ (a máquina deve ter duas fitas)
		
		\item $L = \{w zin \{0,1\}^*: w = w^R\}$ (a máquina deve não determinística e ter duas fitas)		
	\end{itemize}
